...
#sort critical-adevents-2016-09-17-conv.txt | uniq -c >sorted-critical-adevents-2016-09-17-conv.txt  

#delete thode in second pass
#in /tmp
#to get ids for deletion
#awk '!seen[$0]++' ids_critical_adevents-2016-09-17-conv.txt > sorted_critical_ids
##curl -s -XPOST http://criticales.elasticsearch.service.us-east-1.consul:9200/adevents-2016-09-17/conversion/_bulk --data-binary "@sorted_critical_ids"; echo
...
with open ('/tmp/critical-adevents-2016-09-17-conv.txt', 'w') as f:
	f.write ("backup of deleted docs for critical-adevents-2016-09-17\n")
...
def get_17_recs(isohour, ri):
	...
	for doc in data:
		with open ('/tmp/critical-adevents-2016-09-17-conv.txt', 'a') as f: f.write (str(doc)+'\n')
...
def del_17_recs(isohour, ri):
	...
	for doc in data:
		...
		stmt = "{ \"delete\" : { \"_id\" : \"" + tid +"\" } }"
		print stmt
		with open ('/tmp/ids_critical_adevents-2016-09-17-conv.txt', 'a') as f: f.write (str(stmt)+'\n')
